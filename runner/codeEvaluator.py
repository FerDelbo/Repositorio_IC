...
class CodeEvaluator:
    def __init__(self, input_dirctory, output_dirctory, nameExercise, 
                 nameLLM, language, prompt, testcase, temp):
    ...
    def _pathInput(self,):
        ...
        self.base_input_dirctory = os.path.join(diretorio_anterior, 'IC')
    ...
    def run(self, k):
        ...
        newName = codeGenerator.codeRun(self.base_input_dirctory, path, self.nameLLM, self.nameProblems,
                                        self.language, self.prompt_type, session, k, self.temperature)
        # #passo 7 com o código salvo é realizdo os casos de teste
        self._pathInput()
        print(self.base_input_dirctory)
        #passo 8 pega o resultado e traforma em XML
        executeTestCase = testExecute.TestExecute(newName, self.nameLLM, self.prompt_type, self.language,
                                                  session, path, self.base_input_dirctory, self.fileTC, k,
                                                    self.temperature)
        test_results = executeTestCase.runTestCase()
        # #passo 9 é feita uma limpa no XML e pega o conteudo para jogar em uma planilha fora desse repositorio
        # print("\n", test_results)
        extraction.run(newName, self.nameLLM, self.language, self.prompt_type, path, 
                       self.temperature, test_results, k)
        # #programa finalizado
...
parser.add_argument('temperature', type=str, help="teperature")
parser.add_argument('k', type=int, help="execution")


#Analisando os argumentos
args = parser.parse_args()
start = CodeEvaluator(args.input, args.output, args.nameProblems, 
                      args.nameLLM, args.language, args.listPrompt, 
                      args.testCase, args.temperature)
i = 1
while i <= args.k:
    start.run(i)
    i = i + 1